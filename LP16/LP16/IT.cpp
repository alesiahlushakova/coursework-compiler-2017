...
namespace IT
{
	...
	void showTable(IdTable& idtable)
	{
		...
		{
			std::cout << std::setfill('0') << std::setw(2) << std::right << i << " * ";
			std::cout << std::setfill(' ') << std::setw(13) << std::left << idtable.table[i].id << " * ";
			switch (idtable.table[i].iddatatype)
			{
			case RC: std::cout << std::setw(10) << std::left;
				if ((idtable.table[i].idtype == OP) || (idtable.table[i].idtype == STD)) std::cout << "" << " * ";
				else std::cout << "rc" << " * "; break;
			...
			case STD: std::cout << std::setw(18) << std::left << "библиотека" << " * "; break;
				numberOP++;
				break;
			...
			{
				if (idtable.table[i].iddatatype == RC && (idtable.table[i].idtype == V || idtable.table[i].idtype == L))
				{
					std::cout << (idtable.table[i].value.vrc ? "true" : "false");
				}
				else
					std::cout << "-";
			}
