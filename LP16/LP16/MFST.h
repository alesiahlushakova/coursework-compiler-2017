...
#define MFST_TRACE_START(log)	*(log.stream) << std::setw(4) << std::left << "Шаг" << "* " \
									  ...
									  << std::setw(60) << std::left << "Входная лента" \
									  << std::setw(50) << std::left << "Стек" \
									  << std::endl;

#define MFST_TRACE1(log)	*(log.stream)<< std::setw(4) << std::left << ++FST_TRACE_n << "* " \
								  << std::setw(30) << std::left << rule.getCRule(rbuf, nrulechain) \
								  << std::setw(60) << std::left << getCLenta(lbuf, lenta_position) \
								  << std::setw(50) << std::left << getCSt(sbuf) \
								  << std::endl;
...
#define MFST_TRACE4(c,log)	*(log.stream)<< std::setw(4) << std::left << ++FST_TRACE_n << "* " << std::setw(30) << std::left << c <<std::endl; 
#define MFST_TRACE5(c,log)	*(log.stream)<< std::setw(4) << std::left << FST_TRACE_n << "* " << std::setw(30) << std::left << c << std::endl; 
#define MFST_TRACE6(c, k,log)	*(log.stream) << std::setw(4) << std::left << FST_TRACE_n << "* " << std::setw(30) << std::left << c << k << std::endl; 
#define MFST_TRACE7(log)	*(log.stream)<< std::setw(4) << std::left << state.lenta_position << "* " \
								  << std::setw(60) << std::left << rule.getCRule(rbuf, state.nrulechain) \
								  << std::endl;
...
namespace MFST
{
	...
	struct Mfst				// магазинный автомат
	{
		...
		bool savestate( bool need_trace, Log::LOG log );				// сохранить состояние автомата
		bool reststate(bool need_trace, Log::LOG log);				// восстановить состояние автомата
		bool push_chain(				// поместить цепочку правила в стек
			GRB::Rule::Chain chain		// цепочка правила
		);
		RC_STEP step( Log::LOG log, bool need_trace);					// выполнить шаг автомата
		bool start(bool need_trace, Log::LOG log);					// запустить автомат
		bool savediagnosis(
