...
namespace MFST
{
	...
	Mfst::RC_STEP Mfst::step(Log::LOG log, bool need_trace)		// шаг автомата
	{
		RC_STEP rc = SURPRISE;
			...
			if (ISNS(st.top()))			// если на вершине стеке нетерминал
			{
				...
				if ((nrule = grebach.getRule(st.top(), rule)) >= 0)		// если найдено правило по нетерминалу на вершине стека... заполняем номер текущего правила
				{
					...
					if ((nrulechain = rule.getNextChain(lenta[lenta_position], chain, nrulechain + 1)) >= 0)	// получаем следующую цепочку по терминалу из ленты
					{
						...
						savestate(need_trace,log);		// сохраняем состояние
					...
					else		// не найдена подходящая цепочка
					{
						...
						rc = reststate( need_trace,log) ? NS_NORULECHAIN : NS_NORULE;	// восстановить состояние автомата
					};
				}
	...
	bool Mfst::start(bool need_trace, Log::LOG log)
	{
		...
		while (rc_step == NS_OK || rc_step == NS_NORULECHAIN || rc_step == TS_OK || rc_step == TS_NOK) rc_step = step(log, need_trace);
